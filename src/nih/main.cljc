(ns nih.main
  "Kotodama WASM entrypoint for the NIH clean-room actor (L4) — Clojure port.
  ...
  facts (`nih.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"nih.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nih-compat" :tier tier :entities entities} 200])
